...
from code_d2.decor import Sky, Clouds
from code_d2.settings import screen_width
...
class Node(pygame.sprite.Sprite):
    ...
    def update(self):
        if self.status == "a":
            ...
            tint_surf.fill('gray', None, pygame.BLEND_RGB_MULT)
...
class Overworld():
    ...
    def setup_nodes(self):
        ...
        for index, node_data in enumerate(levels.values()):
            if index <= self.max_level:
                node_sprite = Node(node_data['node_pos'], 'a', self.speed, node_data['node_graphics'])
            else:
                node_sprite = Node(node_data['node_pos'], 'l', self.speed, node_data['node_graphics'])
    ...
    def draw_paths(self):
        if self.max_level > 0:
            ...
            pygame.draw.lines(self.display_surface, '#82645A', False, points, 8)
